...
def identify_file(file_path, is_rename, is_preview):
  file_extension = os.path.splitext(file_path)[1]
  if is_file_type_allowed(file_extension):
    shazamapp = ShazamAppTrack(file_path, is_rename, is_preview)
...
def identify_folder_files(directory_path, is_recursive, is_rename, is_preview):
    for root, dirs, files in os.walk(directory_path):
        for filename in files:
            file_path = os.path.join(root, filename)
            identify_file(file_path, is_rename, is_preview)
        if not is_recursive:
            break
